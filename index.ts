...
const isLoggedIn$ = new BehaviorSubject<boolean>(false); //the initial state is false
...
fromEvent(printStateButton, 'click')
  ...
  .subscribe(
    ([event, isLoggedIn]) => console.log('User is logged in:', isLoggedIn) //get the current state
  );
